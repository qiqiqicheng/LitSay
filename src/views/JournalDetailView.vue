<template>
  <div class="journal-detail-container">
    ...
    <!-- 期刊信息 -->
    <div
      v-else-if="containerData && containerData.type === 'journal'"
      class="journal-content"
    >
      <div class="journal-header">
        <h1 class="journal-name">{{ containerData.name }}</h1>
        <div v-if="containerData.journalIssue" class="journal-issue">
          <book-outlined /> {{ containerData.journalIssue }}
        </div>
      </div>
    ...
    if (response.data.code === 0) {
      ...
      if (containerData.value.type !== "journal") {
        ...
        if (containerData.value.type === "conference") {
          router.replace(`/conference/${containerId}`);
          return;
        }
      ...
      document.title = `${containerData.value.name} - 期刊详情 - LitSay`;
    } else {
      error.value = response.data.message || "获取数据失败";
    }
  } catch (err) {
    console.error("获取期刊详情出错:", err);
